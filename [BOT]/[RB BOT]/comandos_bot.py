...
XDDERIO = os.getenv("API")
def buscar_cuenta(ID):
    import requests

    res = requests.post(
        "https://apisss-production.up.railway.app/buscar",
        json={"id": ID}
    )
    data = res.json()
    ...
    if "ID" in data:
        ...
        USER = data["USER"]
        VCM_USER = data["VCM"]
        CLAVE_USER = data["CLAVE"]
        return embed("Cuenta encontrada",
f"""

```ansi
[2;37m[1;37mUSER: {USER}
[1;30m-----------------[0m[1;37m
ID: {ID_USER}
[0m[2;37m[2;30m-----------------
[0m[2;37m[1;37mFECHA: {VCM_USER}
[0m[2;37m[2;30m[2;30m[2;30m-----------------[0m[2;30m[0m[2;30m[0m[2;37m[0m
[2;37m[1;37mCLAVE: {CLAVE_USER}[0m[2;37m[0m
```
...
# --- Setup principal ---
def setup(bot):
    crear_comando('!HELP', 'Muestra los comandos disponibles.')
    crear_comando('!ID', 'Muestra tu id de discord.')
    crear_comando("!LINK", "Muetra el link del servidor")

    @bot.tree.command(name="sugerir", description="Envía una sugerencia.")
    @app_commands.describe(descripcion="Describe tu sugerencia")
    async def sugerencias(interaction: discord.Interaction, descripcion: str):
        canal_id = 1386228857396400188
        ...
        await canal.send(embed=embed(f"{user.display_name} sugirió:", descripcion))
        await interaction.response.send_message("✅ ¡Tu sugerencia fue enviada correctamente!", ephemeral=True)
    ...
    async def user(ctx, *, mensaje: str):
        await ctx.send(embed=buscar_cuenta(mensaje))
    ...
    async def on_raw_reaction_add(payload: discord.RawReactionActionEvent):
        ...
        if emoji == "✅":
            ...
            try:
                ...
                canal = guild.get_channel(1385460631112519755)
                if canal:
                    embede = embed("UN NUEVO MIENBRO", f"✅ {member.mention} Se unio.")
                    await canal.send(embed=embede)
